...
class Cleaner(object):
  ...
  def lrb_rrb(self, text):
    return translate_brackets(text)
  
  def restore_lrb_rrb(self, text):
    return restore_brackets(text)

  def clean(self, text, remove_newspaper_hyphen=False, keep_brackets=True):
    if text:
      self.set_text(text)
    t = self.text.strip()
